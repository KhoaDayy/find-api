...
// Minimal SHA-256 (public domain style) for capture hashing only.
...
inline std::string sha256_hex(const void *data, size_t len) {
  ...
  memcpy(&padded[0], msg, len);
  padded[len] = (char)0x80;
  for (int i = 0; i < 8; i++)
    padded[padlen - 1 - i] = (char)((bitlen >> (8 * i)) & 0xff);

  for (size_t off = 0; off < padlen; off += 64) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
      w[i] = ((uint32_t)(uint8_t)padded[off + i * 4] << 24) |
             ((uint32_t)(uint8_t)padded[off + i * 4 + 1] << 16) |
             ((uint32_t)(uint8_t)padded[off + i * 4 + 2] << 8) |
             ((uint32_t)(uint8_t)padded[off + i * 4 + 3]);
    }
    for (int i = 16; i < 64; i++) {
